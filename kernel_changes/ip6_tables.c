...
static inline bool
icmp6_type_range_match(u_int8_t type, u_int8_t min_type, u_int8_t max_type)
{
...
icmp6_match(const struct sk_buff *skb, struct xt_action_param *par)
{
    ...
    if( icmpinfo->opt_type == O_ICMPV6_TYPE ){
    	return icmp6_type_code_match(icmpinfo->type.type,
	    			     icmpinfo->type.code[0],
		    		     icmpinfo->type.code[1],
			    	     ic->icmp6_type, ic->icmp6_code,
				         !!(icmpinfo->type.invflags&IP6T_ICMP_INV));
    }else if( icmpinfo->opt_type == O_ICMPV6_TYPE_RANGE ){
        return icmp6_type_range_match(ic->icmp6_type, 
                        icmpinfo->range.min_type, 
                        icmpinfo->range.max_type); 
    }else{
        pr_err("not support opt type\n");
        return false;
    }
}
...
static int icmp6_checkentry(const struct xt_mtchk_param *par)
{
	...
	/* Must specify no unknown invflags */
    	return (icmpinfo->type.invflags & ~IP6T_ICMP_INV) ? -EINVAL : 0;
    }else if( icmpinfo->opt_type == O_ICMPV6_TYPE_RANGE ){
        return 0;
    }else{
        return 0;
